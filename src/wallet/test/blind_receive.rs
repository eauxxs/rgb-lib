...
fn success() {
    ...
    assert_eq!(invoice_data.asset_iface, Some(AssetIface::RGB20Fixed));
    ...
    let invoice = Invoice::new(receive_data.invoice).unwrap();
    let invoice_data = invoice.invoice_data();
    assert_eq!(invoice_data.asset_iface, Some(AssetIface::RGB21Unique));
    ...
    let invoice = Invoice::new(receive_data.invoice).unwrap();
    let invoice_data = invoice.invoice_data();
    assert_eq!(invoice_data.asset_iface, Some(AssetIface::RGB25Base));
